...
public class NotMyWallAdapter extends RecyclerView.Adapter<WallViewHolder> {

    private LayoutInflater mInflater;
    private Model model;
    private OnRecyclerViewClickListener onRecyclerViewClickListener;

    public NotMyWallAdapter(Context context, Model model, OnRecyclerViewClickListener onRecyclerViewClickListener){
    ...
    public void onBindViewHolder(@NonNull WallViewHolder holder, int position) {
        String cTitle = model.getNotMyWall().get(position);
        holder.bind(cTitle);
    }

    @Override
    public int getItemCount() {
        return model.getNotMyWall().size();
    }
}
